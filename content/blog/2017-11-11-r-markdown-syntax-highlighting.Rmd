...
In a way this serves as the first part of a code sandwich where you setup the needed source hook, write the SAS code in an R code chunk, and then evaluate the code chunk to apply the highlighting.
...
### `_hilang_eval.Rmd`
Finally, the SAS code needs to have highlighting applied to it.  This is accomplished by identifying the SAS code within `<textarea>` tags and then applying CodeMirror's Javascript to the entire block of text within the `<textarea>` tag.  The file [_highlang_eval.Rmd](https://raw.githubusercontent.com/curtisalexander/rmarkdown-with-sas/master/_hilang_eval.Rmd) contains the following, which when executed applies highlighting.
