...
### Drafts
The [blogdown documentation](https://bookdown.org/yihui/blogdown/content.html#yaml-metadata) suggests adding the YAML field `draft: true` to the YAML front matter of a blog post in order to set the post as a draft.  However, this is only effective if one builds their site using `blogdown` with the command `blogdown::build_site()` or `blogdown::hugo_build()`.  However, I do not utilize either of those commands to build my site.  Instead, I rely on [netlify](https://www.netlify.com) to build my site.

My solution is to manage drafts with directory and `git`.  I create a `drafts` directory where I would like to store any draft `Rmd` files.  Then within my `.gitignore` file, I add entries for the relevant directories.
